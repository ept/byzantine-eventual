...
\begin{description}
\item[Eventual update:] If an update is applied by a correct replica, all correct replicas will eventually apply that update.
...
% TODO: compare to (Byzantine) Lattice Agreement
...
\begin{algorithm}[p]
    ...
    \EndOn
    \State
    \On{receiving $\langle\mathsf{heads}: \mathit{hs},\, \mathsf{oldHeads}: \mathit{oldHeads},\, \mathsf{filter}: \mathit{filter}\rangle$}
        ...
        \EndIf
        \State \Call{HandleMissing}{$\{h \in \mathit{hs} \mid \nexists m \in \mathcal{M}_\mathsf{conn}.\; H(m) = h\}$} \label{line:a2-heads-missing2}
    \EndOn
    \State
    \On{receiving $\langle\mathsf{msgs}: \mathit{new}\rangle$} \label{line:recv-msgs2}
        ...
        \State \Call{HandleMissing}{$\mathit{unresolved}$} \label{line:msgs-handle-missing2}
    \EndOn
    \State
    \On{receiving $\langle\mathsf{needs}: \mathit{hashes}\rangle$} \label{line:recv-needs2}
    ...
    \Function{HandleMissing}{$\mathit{hashes}$}
        \State $\mathit{missing} := (\mathit{missing} \cup \mathit{hashes}) \setminus \{H(m) \mid m \in \mathit{recvd}\}$
        \If{$\mathit{missing} = \{\}$} \label{line:missing-empty2}
...
\begin{algorithm}[p]
    ...
    \EndOn
    \State
    \On{receiving $\langle\mathsf{heads}: \mathit{hs}\rangle$} \label{line:recv-heads3}
        \State $hashes := \{h \in \mathit{hs} \mid \nexists m \in \mathcal{M}_\mathsf{conn}.\; H(m) = h\}$ 
        \State $\mathit{missing} := (\mathit{missing} \cup \mathit{hashes}) \setminus \{H(m) \mid m \in \mathit{recvd}\}$
        \If{$\mathit{missing} = \{\}$} \label{line:missing-empty3}
    ...
    \State
    \On{receiving $\langle \mathsf{oldHeads}: \mathit{oldHeads},\, \mathsf{filter}: \mathit{filter}\rangle$}
    ...
    \EndOn
    \State
    \On{receiving $\langle\mathsf{msgs}: \mathit{new}\rangle$} \label{line:recv-msgs3}
    ...
    \EndOn
    \State
    \On{receiving $\langle\mathsf{needs}: \mathit{hashes}\rangle$} \label{line:recv-needs3}
