...
Then two updates $u_1$ and $u_2$ are commutative if
...
Invariants can represent many types of consistency properties and constraints commonly found in databases, such as referential integrity, uniqueness, or limitations on the value of a data item (e.g.\ requiring it to be non-negative).

Let $u_1$ be the updates resulting from the execution of transaction $T_1$, and $u_2$ the updates from $T_2$.
Assume that $u_1$ and $u_2$ commute, and that these transactions preserve some invariant $I$, that is:
\[ \forall S.\; I(S) \Longrightarrow I(\mathrm{apply}(S, u_1)) \wedge I(\mathrm{apply}(S, u_2)). \]
Then transactions $T_1$ and $T_2$ are \I-confluent with regard to $I$ if
\[ \forall S.\; I(S) \Longrightarrow I(\mathrm{apply}(\mathrm{apply}(S, u_1), u_2)). \]
As $u_1$ and $u_2$ commute, this also implies $I(\mathrm{apply}(\mathrm{apply}(S, u_2), u_1))$.
In words, $T_1$ and $T_2$ are \I-confluent with regard to $I$ if two different replicas can independently execute $T_1$ and $T_2$, each ensuring that $I$ is preserved, and we can be sure that the result of merging the updates from $T_1$ and $T_2$ will still satisfy $I$.

As example, consider a uniqueness constraint, i.e.\ $I(S)=\mathsf{true}$ if there is no more than one data item in $S$ in which a particular attribute has a given value.
If $T_1$ and $T_2$ are both transactions that create data items with the same value in that attribute, then $T_1$ and $T_2$ are not \I-confluent with regard to $I$: each of $T_1$ and $T_2$ individually preserves the constraint, but the combination of the two does not.

As a second example, say that $I(S)=\mathsf{true}$ if every user in $S$ has a non-negative account balance.
If $T_1$ and $T_2$ are both transactions that increase the same user's account balance, then $T_1$ and $T_2$ are \I-confluent with regard to $I$, because the sum of the two positive balance updates cannot cause the balance to become negative (assuming no overflow).
...
Read operations reflect the state of the replica on which they are performed, and reads can be performed at any time.
