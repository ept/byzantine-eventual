...
In contrast, with our Algorithm~\ref{fig:algorithm2}, a reconciliation requires a constant 1.03 round trips on average, regardless of the number of messages since the last reconciliation.
...
For comparison, a 3~kB overhead is 0.15\%.
Thus, we can see that in terms of network performance, our Algorithm~\ref{fig:algorithm2} is close to the optimum (one round trip and zero overhead).
...
Recent years have seen a surge in interest in decentralized peer-to-peer applications, under the umbrella of `web 3'. Such systems aim to enable a collection of different parties to cooperate in the distribution of computation without also distributing their trust to one another. At the time of writing, these applications are being built upon public distributed ledgers (blockchains) which provide a total ordering over events in a byzantine fault tolerant manner, however, application logic is often executed \emph{off-chain} to avoid the high costs and long delays inherent in today's blockchains. 
Byzantine causal broadcast offers a low-cost, lightweight alternative to distributed ledges and the Byzantine atomic broadcast they provide. For some applications, such as cryptocurrencies, causal broadcast alone is not a substitute for atomic broadcast, however, for many other applications, such as group messaging or social media platforms, causal broadcast is sufficient. 
