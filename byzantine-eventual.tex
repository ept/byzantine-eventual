...
\begin{algorithm}[t]
    ...
    \On{delivering $m$ by causal broadcast}\label{line:kv-deliver}
        ...
        \ElsIf{$\exists v', h'.\; (k,v',h') \in S \,\wedge\, (h' \in \{H(m') \mid m' \in \mathrm{pred}^*(\mathcal{M}, m)\} \,\vee\, h' < H(m))$}\label{line:if-overwrite}
...
\section{Proof of Correctness for Byzantine Causal Broadcast}\label{sec:proof}
