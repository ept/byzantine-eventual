...
\begin{minipage}{0.57\linewidth}
    ...
    \On{connecting to another process}
        \State $\mathit{sent} := \{\}; \mathit{received} := \{\}$ \Comment{connection-local variables}
        \State \textbf{send} $\langle\mathsf{heads}: \{H(u) \mid u \in \mathcal{U} \wedge \nexists (v, \mathit{hs}) \in \mathcal{U}.\; H(u) \in \mathit{hs}\}\rangle$
    \EndOn\vspace{6pt}
    \On{receiving $\langle\mathsf{heads}: \mathit{hs}\rangle$}
        \State $\mathit{reply} := \{v \mid \exists u \in \mathcal{U}.\; H(u) \in \mathit{hs} \,\wedge\, v \in \mathrm{succ}^*(u)\}$
        \If{$\mathit{reply} \neq \{\}$}
            \State $\mathit{sent} := \mathit{sent} \cup \mathit{reply};\;$ \textbf{send} $\langle\mathsf{updates}: \mathit{reply}\rangle$
        \EndIf
        \State \Call{HandleMissing}{$\{h \in \mathit{hs} \mid \nexists u \in \mathcal{U}.\; H(u) = h\}$}
    \EndOn\vspace{6pt}
    \On{receiving $\langle\mathsf{updates}: \mathit{new}\rangle$}
        ...
        \State \Call{HandleMissing}{$\mathit{missing}$}
    \EndOn\vspace{6pt}
