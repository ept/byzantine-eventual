...
\begin{algorithm}
    ...
    \On{request to broadcast $v$}\label{line:broadcast-begin}
        \State $\mathit{hs} := \mathrm{heads}(\mathcal{M})$
    ...
    \On{connecting to another process} \label{line:connect-begin}
        \State $\mathit{recvd} := \{\};\; \mathit{missing} := \{\};\; \mathcal{M}_\mathsf{conn} := \mathcal{M}$ \label{line:init}\Comment{connection-local variables}
    ...
    \On{receiving $\langle\mathsf{needs}: \mathit{hashes}\rangle$ via a connection} \label{line:recv-needs}
        \State $\mathit{reply} := \{m \in \mathcal{M}_\mathsf{conn} \mid H(m) \in \mathit{hashes}\}$ \label{line:needs-reply}
...
% FIXME (mk): I think we need to bring back the \mathit{sent} variable after all
...
\begin{algorithm}[t]
    ...
    \On{connecting to process $q$}\Comment{Replaces lines \ref{line:connect-begin}--\ref{line:connect-end} of Algorithm \ref{fig:algorithm}}
        \State $\mathit{recvd} := \{\};\; \mathit{missing} := \{\};\; \mathcal{M}_\mathsf{conn} := \mathcal{M}$ \Comment{connection-local variables}
...
\section{Byzantine Eventual Consistency and CRDTs}\label{sec:byzantine-crdts}
...
BEC is not strong enough to implement a cryptocurrency, since it cannot prevent double-spending, but it is sufficient for many applications that currently use CRDTs, as listed above.
