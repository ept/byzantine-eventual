...
\begin{enumerate}
    ...
    The first message from process $q$ to process $p$ will be a head message with $\mathit{heads}(\mathcal{U}_q)$ (Fig.~\ref{fig:algorithm}, line~\ref{line:send-heads}).\\
    Since $u \in heads(\mathcal{U}_q)$ and $u \notin  \mathcal{U}_p$ then process $p$ will send an needs message to process $q$ including $H(u)$ (Fig.~\ref{fig:algorithm}, line~\ref{line:heads-missing}).\\
    Upon receiving the needs message including $H(u)$, process $q$ will reply with an update message including $u$ (Fig.~\ref{fig:algorithm}, line~\ref{line:send-updates}).\\
    Process $p$ will receive the update message with $u$ from process $q$ and will add $u$ to $\mathit{received}$ (Fig.~\ref{fig:algorithm}, line~\ref{line:updates-received}).\\
