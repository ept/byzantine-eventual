...
In this paper, we characterise this category of applications using a consistency model we call \emph{Byzantine Eventual Consistency} (BEC).
We introduce an algorithm that guarantees BEC, prove its correctness, and demonstrate good performance in a prototype implementation, making our algorithm viable for practical use.
Our algorithm is based on a Byzantine fault tolerant causal broadcast protocol.
\end{abstract}
\maketitle
...
We do assume that a replica has a timer that allows it to perform some task approximately periodically, without requiring exact time measurement.
...
Let $p$ be the correct replica that executes $T_1$, let $q$ be the correct replica that executes $T_2$, and assume that all of the remaining replicas $R \setminus \{p,q\}$ are Byzantine-faulty.
...
Each connection thread executes independently, except for the blocks marked \emph{atomically}, which are executed only by one thread at a time.
...
\begin{algorithm}
    ...
    \On{delivering $m$ by causal broadcast}
        ...
        \If{updates $(\mathit{ins}, \mathit{del})$ are safe w.r.t. all invariants and\label{line:safety-check}\\\hspace{3em}$\forall (h,r,t) \in \mathit{del}.\; h \in \{H(m') \mid m' \in \mathrm{pred}^*(\mathcal{M}, m)\}$}\label{line:pred-check}
...
\subsection{BEC replication using causal broadcast}

Given the Byzantine causal broadcast protocol we have defined, we now introduce a replication algorithm that ensures Byzantine Eventual Consistency and preserves invariants, assuming \I-confluence.
The details depend on the data model of the database being replicated, and the types of updates allowed.
Algorithm~\ref{fig:algorithm3} shows an example for a relational database that supports insertion and deletion of tuples in relations (updates can be represented by deleting the old tuple and inserting the new one).

Let each replica have state $S$, which is stored durably, and which is initially the empty set.
$S$ is a set of triples: $(h, \mathit{rel}, \mathit{tuple}) \in S$ means the relation named $\mathit{rel}$ contains the tuple $\mathit{tuple}$, and that tuple was inserted by a message with hash $h$.
We assume the schema of $\mathit{tuple}$ is known to the application, and we ignore DDL in this example.
Our algorithm gives each replica a full copy of the database $S$; sharding could be added if required.
...
We assert that it is possible to determine safety unambiguously for all common types of constraint, but we leave a detailed analysis of this matter for future work.
...
This ensures that subsequent deletions can unambiguously reference the element of $S$ to be deleted, even if multiple replicas concurrently insert the the same tuple into the same relation.

This algorithm could be extended with other operations besides insertion and deletion: for example, it might be useful to support an operation that adds a (possibly negative) value to a numeric attribute; such an operation would commute trivially with other operations of the same type.
Further data models can be implemented using CRDT techniques (\S~\ref{sec:crdts}).
...
\begin{proof}
Consider first the conditions on lines~\ref{line:safety-check} and~\ref{line:pred-check} of Algorithm~\ref{fig:algorithm3}; by Lemma~\ref{lemma:safety-check}, these conditions always evaluate to the same value for a given message, regardless of the order in which the messages are delivered.
...
If the conditions are $\mathsf{true}$ for both messages, TODO\dots
\end{proof}

\begin{theorem}
Algorithm~\ref{fig:algorithm3} ensures Byzantine Eventual Consistency, as defined in \S~\ref{sec:bec-definition}.
\end{theorem}
...
Moreover, we must show that one correct replica applies the updates in a message if and only if another correct replica applies it.
...
It is therefore possible to permute $p$'s delivery sequence into $q$'s delivery sequence by repeatedly swapping adjacent messages in that sequence, and neither message is a predecessor of the other.
If we can show that each such swap of adjacent messages leaves the final state of the replica unchanged, then we have demonstrated that $p$ and $q$ must be in the same state.
This follows from Lemma~\ref{lemma:commutative}.
\end{proof}

\begin{theorem}
Assume that the safety check in Algorithm~\ref{fig:algorithm3} only allows updates from transactions that are \I-confluent with regard to all of the application's invariants.
Then Algorithm~\ref{fig:algorithm3} ensures that the state of each correct replica always satisfies all of the application's invariants.
\end{theorem}
...
% Potential follow-on work: algorithms to determine whether an update is safe with regard to commonly used invariants.
