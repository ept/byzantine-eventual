...
In addition to defining BEC, we present two novel broadcast algorithms which implement \emph{Byzantine Causal Broadcast} (\S~\ref{sec:algorithm}). 
...
A total order broadcast (a.k.a.\ \emph{atomic broadcast}) protocol is defined in terms of the two primitives, \emph{broadcast} and \emph{deliver}, and must satisfy the following properties:
...
However, additional mechanisms are required to ensure casual ordering, the fifth property of causal broadcast, and this algorithm is very inefficient.
...
A reconciliation algorithm should determine which messages have already been delivered by both processes, and transmit only those messages that are unknown to the other process.
...
Ideally, a reconciliation algorithm should also complete in a small number of round-trips and minimise the size of messages send.
These goals rule out other na\"{\i}ve approaches too, for example, where process $p$ sends a set of message hashes which can be used by other processes to determine which messages they are missing, since the size of this message is still $O(|\mathcal{M}_{p}|)$.
...
Let the set of delivered messages $\mathcal{M}$ be a set of triples $(v, \mathit{hs}, \mathit{sig})$, where $v$ is any value, $\mathit{sig}$ is a digital signature over $(v, \mathit{hs})$ using the sender's private key, and $\mathit{hs}$ is a set of hashes produced by a cryptographic hash function $H(\cdot)$, such as SHA-256.
...
\algblockdefx{Mutex}{EndMutex}{\textbf{within a per-process mutex do}}{\textbf{end mutex}}
\begin{algorithm}
    \begin{algorithmic}[1]
    \On{connecting to another process} \label{line:connect-begin}
        ...
        \State \textbf{send} $\langle\mathsf{heads}: \mathrm{heads}(\mathcal{M}_\mathsf{conn})\rangle$ \label{line:send-heads}
    \EndOn \label{line:connect-end}
    \State
    \On{receiving $\langle\mathsf{heads}: \mathit{hs}\rangle$} \label{line:recv-heads}
        \State $\mathit{reply} := \{m' \mid \exists m \in \mathcal{M}_\mathsf{conn}.\; H(m) \in \mathit{hs} \,\wedge\, m' \in \mathrm{succ}^*(\mathcal{M}_\mathsf{conn}, m) \,\wedge\, m' \notin \mathit{sent}\}$ \label{line:succ}
        \If{$\mathit{reply} \neq \{\}$}
            \State $\mathit{sent} := \mathit{sent} \cup \mathit{reply}$
            \State \textbf{send} $\langle\mathsf{msgs}: \mathit{reply}\rangle$ \label{line:heads-reply}
        \EndIf
    ...
    \State
    \On{receiving $\langle\mathsf{msgs}: \mathit{new}\rangle$} \label{line:recv-msgs}
    ...
    \On{receiving $\langle\mathsf{needs}: \mathit{hashes}\rangle$} \label{line:recv-needs}
        ...
        \State $\mathit{sent} := \mathit{sent} \cup \mathit{reply}$
        \State \textbf{send} $\langle\mathsf{msgs}: \mathit{reply}\rangle$ \label{line:send-msgs}
...
In order to provide casual ordering and to keep the number of heads small, whenever a correct process broadcasts a new message $(v, \mathit{hs}, \mathit{sig})$, it should set $\mathit{hs}$ to be the set of hashes of the current heads:
...
\begin{algorithm}[t]
    ...
    \EndOn
    \State
    \Function{MessagesSince}{$\mathit{oldHeads}$}
...
\end{description}
% TODO: add a causal order property? something like: "if a correct replica generates or applies update $u_1$ before generating update $u_2$, then all replicas apply $u_1$ before $u_2$"
% But then it would be better to call that model "Byzantine causal consistency" rather than "Byzantine eventual consistency"
