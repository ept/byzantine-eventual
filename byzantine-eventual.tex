...
In these experiments we test the scenario illustrated in Figure~\ref{fig:evaluation-dags}.
Two nodes $p$ and $q$ are assumed to have the first $n$ messages (in a linear hash chain) in common.
Then $p$ adds a further $n$ messages, and $q$ independently adds another $n$ messages.
...
TODO work in progress\dots


To evaluate the algorithms introduced in Section~\ref{sec:algorithm} we implemented a prototype and measured its behavior.
...
\section{Applications of Byzantine Causal Broadcast}\label{sec:crdts}
...
Recent years have seen a surge in interest in decentralized peer-to-peer applications, under the umbrella of `web 3'. Such systems aim to enable a collection of different parties to cooperate in the distribution of computation without also distributing their trust to one another. At the time of writing, these applications are being built upon public distributed ledgers (blockchains) which provide a total ordering over events in a byzantine fault tolerance manner, however, applications logic is often executed \emph{off-chain} to avoid the high costs and long delays inherent in today's blockchains. 
Byzantine causal broadcast offers a low-cost, lightweight alternative to distributed ledges and the Byzantine atomic broadcast they provide. For some applications, such as cryptocurrencies, casual broadcast alone is not a sufficient substitute for atomic broadcast. However, for many applications, causal broadcast is sufficient, the most obvious examples are group messaging or social media applications. 
...
As discussed in Section~\ref{sec:crdts}, many existing systems and applications use CRDTs to achieve strong eventual consistency in a non-Byzantine model.
