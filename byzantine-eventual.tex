...
Let $S' = \mathrm{apply}(S, u)$ be the function that applies update $u$ to the replica state $S$, resulting in an updated replica state $S'$.
...
For systems that currently require all nodes to be trusted, and hence can only deployed in trusted datacenter networks, adding Byzantine fault tolerance opens up new opportunities for deployment in untrusted peer-to-peer settings.
