...
Unfortunately, vector clocks are not safe to use in the presence of Byzantine nodes, as shown in Figure~\ref{fig:vectorclocks}.
...
Subsequently, when correct nodes $p$ and $q$ exchange vector clocks, they may believe that they have delivered the same set of updates because their vector clocks are identical, even though in fact they have delivered different updates.

Even if updates are signed, this type of misbehaviour by Byzantine nodes cannot be ruled out.
The duplicate sequence numbers might later be detected, but this would require an additional protocol on top of vector clocks to ensure eventual delivery.
...
This section lays out a general-purpose approach for CRDTs to tolerate Byzantine faults.
...
When updates are generated concurrently and subsequently merged, the next update contains multiple predecessors.
...
This is not a problem: it simply results in the update with the missing dependency never being delivered.
...
To avoid the storage cost of explicitly materialising all of the hashes, it would be possible to design a compression scheme that recomputes hashes when necessary.
